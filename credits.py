...
class CreditManager:
    ...
    async def add_credits(self, user_id: int, amount: int):
        async with self.pool.acquire() as conn:
            ...
            if exists:
                ...
                print(f"✅ New user {user_id} inserted with {amount} credits.")
    ...
    async def refill_if_due(self, user_id: int):
        async with self.pool.acquire() as conn:
            ...
            if not row:
                print(f"❌ User {user_id} not found in DB — skipping refill.")
                return None  # Do NOT insert here anymore
            ...
            if not initial_bonus_given and credits < 300:
                ...
                print(f"💥 Bonus: Granted 300 credits to user {user_id}")
            ...
            if time_since >= REFILL_INTERVAL:
                ...
                print(f"🔁 Refilled {REFILL_AMOUNT} credits for user {user_id}")
                return f"💖 You’ve received {REFILL_AMOUNT} free Ava Credits! Enjoy your time again 😘"

            print(f"⏳ User {user_id} is not eligible for refill yet.")
            return None
